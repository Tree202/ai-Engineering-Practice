...
# AW / 失败 / check 都已收进 run_checks —— 模块级不再有可变状态,
# 所以 import 这个模块是安全的,同一进程里也能反复调用。


def read(p):
    # 用 with:不关句柄在正常跑一遍时无所谓(进程马上退出),
    # 但测试会在同一进程里反复调用,ResourceWarning 会刷屏,
    # 而且 Windows 上没关的句柄会让 tempdir 清理失败。
    with io.open(p, encoding="utf-8", errors="replace") as fh:
        return fh.read()


def run_checks(root=None):
    """跑一遍全部口径断言,返回 0(全平)或 1(有不平)。

    2026-09-03 可测化改造:原来这一整段是**模块级顶层代码**,而且末尾是裸的
    sys.exit —— 任何 import 都会直接终止进程,根本没法写测试。现在收进函数、
    参数化 root、失败清单改成局部(原来是模块级全局,一个进程里跑两次会累加)。
    逻辑一行没动,只是搬了位置。
    """
    root = root or ROOT
    ...
    卡片 = [(m.group(1), int(m.group(2))) for m in
            re.finditer(r'href="(\d\d)-[a-z0-9-]+\.html">(?:(?!</a>).)*?含 (\d+) 处修正', t00, re.S)]
    # 字符类必须含数字:第 10 页的文件名是 10-flaky-e2e.html,里面那个 e2e 带数字,
    # 原来写 [a-z-]+ 匹配不上 —— 那一页的卡片从来没被这条断言碰过。
    # 又一个「检查器没在查它以为在查的东西」,和这一轮修的三个是同一族。
    ...
    # ---------------- 4. kb:知识点计数 = 实际章节数 ----------------
    kb0 = read(os.path.join(ROOT_, "kb", "00-index.html"))
    声明 = [int(m) for m in re.findall(r"(\d+) 个知识点", kb0)]
    实际 = [len(re.findall(r'<h2 id="s\d+"', read(f)))
           for f in sorted(glob.glob(os.path.join(ROOT_, "kb", "0[1-7]*.html")))]
